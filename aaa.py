...
class StockTableView(QWidget):
    ...
    def initUi(self):
        ...
        column_headers = ["Data", "Para", "Giełda", "Ilość", "Cena Zakupu", "Wartość Zakupu", "Aktualna Cena", "Aktualna Wartość"]
            ...
            if gielda == "Zonda":
                ...
                try:
                    ...
                    if success and act_price and act_value is not None:
                        ...
                        print(success, act_price, act_value)
                        # self.stock_table.item(row, 6).setText(str(act_price_dumb))
                        item = self.stock_table.item(row, 7)
                        if item is not None:
                            print(item)
                            item.setText(str(act_value))
                        else:
                             print(f"Brak elementu w tabeli dla wiersza {row} i kolumny 7")
                    else:
    ...
    def load_stock_data(self):
        ...
        cursor.execute("""SELECT 
                        purchase.date AS date, pairs.name AS pair, stocks.name AS name, purchase.count AS count, purchase.price AS price, purchase.value AS value
                        FROM pairs
                        JOIN stocks ON stock_id = stocks.id
                        JOIN purchase ON pairs.id = purchase.purch_pairs""")
